...
// Uses variadic function with optional "debug", parameter

// Variadic parameters are declared in the function parameters section
// e.g. func foo(params ...int) {
// and are accessible as a slice
...
func main() {
	start := time.Now()
//	evalBemdas("2234*((1+2)^2 + (3-5)*2)", "debug")
	fmt.Println("2234*((1+2)^2 + (3-5)*2)")
	fmt.Println(evalBemdas("2234*((1+2)^2 + (3-5)*2)"))
	elapsed := time.Since(start)
	fmt.Printf("%s elapsed\n", elapsed)
	
}

func evalBemdas(arg ...string) float64 {

	// input
	// str := "2234*((1+2)^2 + (3-5)*2)"
	str := arg[0]
	
	// if second argument is "debug" then print stages
	debug := false
	if len(arg)>1 && "debug" == arg[1] { 
		debug = true
	}
